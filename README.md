...
그래프, FC400, 카메라 인식 조건과 MR-MC240N 연결처럼 시험 중 자주 바꾸지
않는 항목은 메인 화면의 `장비 / 그래프 설정` 버튼에서 탭별로 설정합니다.
영점, 모션 Arm/정지, 카메라 개방·녹화와 시험 시작은 메인 화면에 남아 있습니다.
...
`6축 연계 제어 (3+3 직선보간)`도 기본으로 켜집니다. 개별 축 제어가 필요할 때만
메인 화면에서 이 옵션을 해제하세요.
보드 ID와 DLL 설정을 확인한 뒤, 메인 화면의 시험 시작 버튼 바로 위에 있는
`240N 연결 + 프로젝트 적용`을 누르면 연결과 CTR 프로젝트 파라미터 적용,
System Start를 한 번에 진행합니다. 보드 재기동/파라미터 적용 확인 창은 유지되며,
서보 ON·홈 복귀·축 이동은 자동 실행하지 않습니다. 시험/축 동작 중이나
컨트롤러 정리 실패 상태에서는 이 버튼을 사용할 수 없습니다.
설정 창의 `Connect PCIe Board`는 연결만 확인할 때 사용합니다. USB 직접
제어는 PCIe 런타임을 사용할 수 없을 때 사용자가 명시적으로 선택하는 보조
경로입니다. PCIe System Start 자동 실행은 기본적으로 꺼져 있으며, 앱을
실행하는 것만으로 보드에 프로젝트를 적용하거나 축을 움직이지 않습니다.
...
3. 6채널 RSE 입력은 각 FC400 전압 출력을 `AI0`~`AI5`에 연결하고,
   공통 `COM`을 `AI GND`에 연결합니다. 축 1~6은 AI0~AI5에 순서대로 대응합니다.
4. 앱에서 `FC400 + USB-6002`를 선택합니다.
5. 실제 FC400 설정에 맞춰 Zero/Full-scale 전압, 용량, 단위와 샘플 속도를
   입력합니다.

기본값은 `0 V = 0 N`, `10 V = 1000 N`, `Dev1/ai0:5` RSE,
`1000 S/s`입니다. 6개 입력은 축별 하중으로 독립 기록됩니다.

앱 실행 시 `DAQ 실시간`이 켜지며, 시험 전후와 수동 JOG 중에도 표의
6채널 전압(`DAQ [V]`)과 하중을 100 ms마다 갱신합니다. 영점 설정과 시험은
동일한 DAQ 작업을 공유합니다. 시험 기록을 탐색하는 동안에는 그래프를
유지하며, `실시간 보기`로 현재 하중 그래프로 돌아갈 수 있습니다.
`영점조절 (Offset)`은 현재 6채널 값을 새 영점 기준으로만 설정합니다. 기존 시험의
시계열·스트로크 결과·시작 시각·시료 결과와 선택한 이력 그래프는 유지하며,
이미 기록된 값을 새 Offset으로 다시 계산하지 않습니다. 영점 읽기에 실패해도
기존 Offset과 기록을 지우지 않습니다. 시험 중에는 영점을 변경할 수 없습니다.
DAQ 오류가 나면 마지막 수신값임을 표시하고 갱신을 멈춥니다.
원인을 해결한 뒤 설정의 `NI 기기 새로고침`으로 다시 연결할 수 있습니다.

화면의 `Raw Data`, `Zero Offset`, `DAQ [V]`는 원래 부호를 유지합니다.
`Calibrated Value`와 실시간 그래프에는 영점 보정 후 절댓값 `abs(Raw - Zero)`를
표시하며, 기록 탐색 그래프도 저장된 보정값의 절댓값을 표시합니다.
새 시험의 시계열 및 결과에도 화면과 같은 보정값을 저장하며, Raw 기록은 부호를
유지합니다. 전압→하중 환산계수와 과거 기록은 변경하지 않습니다.
기존에 0으로 기록된 보정값은 자동 복원되지 않습니다.
...
현재 소프트웨어 입력 범위는 BTK1404-3.6ZZ+196LC7 구성을 전제로
`0~196 mm`, USB 이동 속도 `1~12,000 mm/min`, 가속/감속
`0~20,000 ms`입니다. 상대 이동과 JOG는 같은 위치 소프트 리미트를
확인합니다. 이 값은 실제 조립 후 측정한 유효 Stroke와 하드 리미트 위치에
맞춰 반드시 재검증해야 합니다.
...
저장합니다. 설정에서 자동 녹화를 켜면 시험 시작/종료와 연동되며
`D:\video`에 저장됩니다. 수동/자동 녹화 모두 저장 폴더를 자동으로 만들며
파일명에는 촬영 시작 시각, 관리번호, 품번,
품명, 시료번호, Jig 크기와 링 색상이 포함됩니다.

## 안전 하중 및 결과 검토

각 스트로크 결과는 최대 길이에서 설정된 홀드 시간이 지난 뒤, 복귀 명령을
내리기 직전 DAQ 갱신에서 읽은 마지막 6채널 샘플입니다. 이동 중 최대값이나
홀드 구간 최대값/평균값이 아니며, 홀드가 0초여도 도착 확인 다음 갱신에서
측정한 뒤 복귀합니다. 갱신 주기는 100 ms이고, 위치도 설정값 대신 당시 피드백을
저장합니다. CSV 요약·시료 결과·PDF는 이 홀드 종료값을 공통으로 사용합니다.
전체 시계열은 계속 보존하며, 결과로 선택한 행은 `State=HOLDING_MAX`,
`Result Capture=HOLD_END`로 표시합니다. 홀드 완료 전 중단하면 해당 스트로크
결과를 만들지 않습니다. 완료된 홀드가 하나도 없으면 CSV에 시계열만 저장합니다.

`안전 하중 리미트`는 목표 하중보다 크고 FC400 설정 용량 이하로 설정해야
합니다. 앱은 USB-6002에서 한 번에 읽은 버퍼의 마지막 값뿐 아니라 모든
샘플의 절대 피크를 검사합니다. 리미트 이상이면 모션 상태 갱신 전에 Axis
1~6 전체에 Rapid Stop을 시도하고 시험을 중단하며 안전 이벤트를 로그와
시계열 데이터에 남깁니다.

이 기능은 Windows 애플리케이션 기반의 보조 인터록입니다. 독립된
하드와이어 비상정지, 앰프/STO 및 기구 리미트를 대체하지 않습니다.

시험 종료 후 방사형 그래프 아래의 `시간 순 / 위치 순` 슬라이더로 기록된
시점의 6축 하중 분포를 정지 화면으로 확인할 수 있습니다. 완료된 시험은
`시료 결과 모음`에 자동 누적되며, 행을 선택하면 선택 시료를, 선택하지 않으면
누적 시료 전체를 시료당 한 페이지의 단일 PDF로 출력합니다.
PDF는 실제 측정 행(최대 10개)과 전체 스트로크 통계를 표시하며, 빈 행과 빈 비고
박스를 만들지 않습니다. 상단 서명 칸은 칸당 약 29 × 23 mm의 서명 공간을
확보하고 기본 정보 표와 겹치지 않도록 분리합니다.
